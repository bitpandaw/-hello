<template>
  <div>
    ...
    <el-table v-loading="ld" :data="rows" border style="width: 100%">
      ...
      <el-table-column prop="orderNo" min-width="160" label="单号" />
    ...
    >
      ...
      <template #footer
        ...
        ><el-button type="primary" :loading="saving" @click="doShip">确定</el-button></template
      >
    </el-dialog>
  </div>
</template>
...
async function load() {
  ld.value = true
  const t = await pageOms(p.value, s.value, st.value)
