    ...
    <el-table v-loading="ld" :data="rows" border>
      ...
      <el-table-column width="120" label="操作">
        <template #default="{ row }">
          <el-button type="warning" text @click="toggle(row)">{{ row.status === 1 ? '封禁' : '解封' }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
async function load() {
  ld.value = true
  const t = await pageUms(p.value, s.value, q.value || undefined, phone.value || undefined)
...
async function toggle(row) {
  ...
  try {
    await ElMessageBox.confirm('确认执行该操作吗？', '提示', { type: 'warning' })
  } catch {
    return
  }
